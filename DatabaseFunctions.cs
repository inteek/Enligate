...
namespace sw_EnligateWeb.Engine
{
    public class DatabaseFunctions
    {
        ...
        {
            return dbApp.Users.Where(u => u.Email.Equals(email)).FirstOrDefault();
        }
        ...
        public bool setUpdateEmailValidation(ApplicationUser usr, string code)
        {
            try
            {
                ...
                user.usuEmailValidationCodeEndDateUtc = DateTime.UtcNow.AddDays(10);
                dbApp.SaveChanges();
        ...
        public bool setUpdatePasswordRecoveryCode(ApplicationUser usr, string code)
        {
            try
            {
                ...
                user.usuPasswordRecoveryCodeEndDateUtc = DateTime.UtcNow.AddDays(10);
                dbApp.SaveChanges();
        ...
        public List<RequestLeaguesViewModel> getLeaguesRequestNew()
        {
            return (from lig in dbApp.tblLigas
                    ...
                    select new RequestLeaguesViewModel()
                    {
                        ...
                        ligFormaPago = lig.tblTarifasPeriodicidad.tblTarifasConceptoFormaPago.tfpIdFormaPago,
        ...
        public List<LeaguesActiveLOwnerViewModel> getRefereeLeagues(string userId)
        {
            ...
            ligas = (from arb in dbApp.tblArbitros                     
                     join arbLig in dbApp.tblArbitrosLigas on arb.arbId equals arbLig.arbId into arb_Lig
                     from arbLig in arb_Lig.DefaultIfEmpty()
                     join lig in dbApp.tblLigas on arbLig.ligId equals lig.ligId
        ...
        {
            return (from lca in dbApp.tblLigaCoAdministradores
                    let prof = dbApp.tblUsersProfiles.Where(p => p.uprPerfilPrincipal == true && p.userIdOwner == lca.lcaUserId).FirstOrDefault()
                    ...
                    {
                        lcaUserId = lca.tblUsuario.Id,
                        lcaNombre = nombreCompleto,
                        lcaEmail = lca.tblUsuario.UserName,
        ...
        /// <returns></returns>
        public bool setLeagueCoadmin(schemaLigaCoAdministradores ligaCoAdmin)
        {
            try
            {
                dbApp.tblLigaCoAdministradores.Add(ligaCoAdmin);
        ...
        public bool setDeleteCoadmin(int ligId, string key, string userId)
        {
            try
            {
                var coadmin = dbApp.tblLigaCoAdministradores
                                   .Where(m => m.ligId == ligId && m.tblLigas.ligUserIdCreador == key && m.lcaUserId == userId).FirstOrDefault();
                if (coadmin != null)
                {
                    dbApp.tblLigaCoAdministradores.Remove(coadmin);
                    dbApp.SaveChanges();
                    return true;
                }
            }
        ...
        public bool setDeleteCoadmin(string userEmail, string code)
        {
            ...
            {
                var coadmin = dbApp.tblLigaCoAdministradores
                                   .Where(m => m.tblUsuario.UserName == userEmail && m.lcaCodigoConfirmacion == code)
                                   .FirstOrDefault();
                if (coadmin != null)
                {
                    dbApp.tblLigaCoAdministradores.Remove(coadmin);
        ...
        public schemaLigas getLigaByCoAdminCodeEmail(string userEmail, string code)
        {
            ...
            {
                var coadmin = dbApp.tblLigaCoAdministradores
                                   .Where(m => m.tblUsuario.UserName == userEmail && m.lcaCodigoConfirmacion == code).FirstOrDefault();
                if (coadmin != null)
                {
        ...
        /// <param name="code"></param>
        /// <returns></returns>
        public bool setConfirmCoadmin(ApplicationUserManager userManager, ApplicationUser user, string code)
        {
            try
            {
                var coadmin = dbApp.tblLigaCoAdministradores
                                   .Where(m => m.tblUsuario.UserName == user.Email && m.lcaCodigoConfirmacion == code)
                                   .FirstOrDefault();
                if (coadmin != null)
                {
                    coadmin.lcaFechaConfirmacionUTC = DateTime.UtcNow;
        ...
        {
            DateTime fechaActual = DateTime.UtcNow.AddDays(-1);
        ...
        {
            var fechaActual = DateTime.UtcNow.AddDays(-1);
        ...
        /// <returns></returns>
        public List<TorneosGridViewModel> getTorneosParaGridByLiga(int? ligId, string userIdCreadorLiga , ApplicationUser user)
        {
            ...
            }else
            {
                torneos = (from tor in dbApp.tblTorneos
                           where (tor.ligId == (int)ligId
                               && tor.tblLiga.ligUserIdCreador == userIdCreadorLiga)
                           select tor)
                           .Distinct()
                           .ToList();
            }
            ...
            return (from tor in torneos
                    let equiposInscritos = dbApp.tblEquipos.Where(e => e.torId == tor.torId && e.equEstatus == true)
                                                    .Where(e => e.equPagado == true || e.equFechaVencimientoPagoUTC > DateTime.UtcNow).Count()
                    let jugadoresInscritos = (tor.torEsCoaching == true) ? dbApp.tblJugadorEquipos.Where(e => e.torId == tor.torId && e.jugEstatus == true).Where(e => e.jugPagado == true || e.jugFechaVencimientoPagoUTC > DateTime.UtcNow).Count() : dbApp.tblJugadorEquipos.Where(e => e.torId == tor.torId && e.equId == 0 && e.jugEstatus == true)
                                                    .Where(e => e.jugPagado == true || e.jugFechaVencimientoPagoUTC > DateTime.UtcNow).Count()
        ...
        public List<TorneosGridViewModel> getTorneosParaGridByArbitro(string userId)
        {
            ...
            return (from arb in arbtitros
                    join tor in dbApp.tblTorneos on arb.ligId equals tor.ligId
                    let equiposInscritos = dbApp.tblEquipos.Where(e => e.tblTorneos.ligId == arb.ligId && e.equEstatus == true)
                                                           .Where(e => e.equPagado == true || e.equFechaVencimientoPagoUTC > DateTime.UtcNow).Count()
                    let jugadoresInscritos = dbApp.tblJugadorEquipos.Where(e => e.torId == arb.ligId && e.equId == 0 && e.jugEstatus == true)
                                                               .Where(e => e.jugPagado == true || e.jugFechaVencimientoPagoUTC > DateTime.UtcNow).Count()
                    let tieneEquipos = (tor.torTipo == constClass.torTipoCoaching || !tor.torDeporteEnEquipo) ? false : true
                    let comentarios = dbApp.tblTorneoComentarios.Where(t => t.torId == tor.torId && t.tcoEstatus == true).ToList()
                    let sumaCalificacionComentarios = (comentarios.Count > 0) ? comentarios.Sum(c => c.tcoCalificacion) : 0
                    ...
                    {
                        torId = tor.torId,
                        ligNombre = arb.tblLigas.ligNombreLiga,
                        torImg = tor.torImgUrl,
                        torComentarios = tor.torComentarios,
                        torNombre = tor.torNombreTorneo,
                        tblCategoria = tor.tblCategoriaTorneo,
        ...
        /// <param name="UserIdCreator"></param>
        /// <returns></returns>
        public List<TorneosCoAdministradoresViewModel> getTorneoCoadministradoresById(int torId, string UserIdCreator)
        {
            return (from tca in dbApp.tblTorneoCoAdministradores
                    let prof = dbApp.tblUsersProfiles.Where(p => p.uprPerfilPrincipal == true && p.userIdOwner == tca.tcaUserId).FirstOrDefault()
                    let nombreCompleto = (prof != null) ? (prof.uprNombres + " " + prof.uprApellidos).Replace("-", "").Trim() : ""
                    where tca.torId == torId
                       && tca.tblTorneo.torUserIdCreador == UserIdCreator
                    select new TorneosCoAdministradoresViewModel()
                    {
                        tcaUserId = tca.tblUsuario.Id,
                        tcaNombre = nombreCompleto,
                        tcaEmail = tca.tblUsuario.UserName,
        ...
        public bool setTorneoCoadminConfirmCode_Update(schemaTorneoCoAdministradores model, string code)
        {
            try
            {
                var coadmin = dbApp.tblTorneoCoAdministradores.Where(m => m.torId == model.torId && m.tcaUserId == model.tcaUserId).FirstOrDefault();
        ...
        /// <param name="userId"></param>
        /// <returns></returns>
        public bool setTorneoCoadmin_Delete(int torId, string key, string userId)
        {
            try
            {
                var coadmin = dbApp.tblTorneoCoAdministradores
                                   .Where(m => m.torId == torId && m.tblTorneo.torUserIdCreador == key && m.tcaUserId == userId).FirstOrDefault();
                if (coadmin != null)
                {
        ...
        public bool setTorneoCoadmin_Delete(string userEmail, string code)
        {
            ...
            {
                var coadmin = dbApp.tblTorneoCoAdministradores
                                   .Where(m => m.tblUsuario.UserName == userEmail && m.tcaCodigoConfirmacion == code)
                                   .FirstOrDefault();
                if (coadmin != null)
                {
                    dbApp.tblTorneoCoAdministradores.Remove(coadmin);
        ...
        public schemaTorneos getTorneoByCoAdminCodeEmail(string userEmail, string code)
        {
            ...
            {
                var coadmin = dbApp.tblTorneoCoAdministradores
                                   .Where(m => m.tblUsuario.UserName == userEmail && m.tcaCodigoConfirmacion == code).FirstOrDefault();
                if (coadmin != null)
                {
        ...
        public bool setTorneoConfirmCoadmin(ApplicationUserManager userManager, ApplicationUser user, string code)
        {
            ...
            {
                var coadmin = dbApp.tblTorneoCoAdministradores
                                   .Where(m => m.tblUsuario.UserName == user.Email && m.tcaCodigoConfirmacion == code)
                                   .FirstOrDefault();
                if (coadmin != null)
                {
                    coadmin.tcaFechaConfirmacionUTC = DateTime.UtcNow;
        ...
        public EquiposJugadoresTotalTorneos getEquiposJugadoresTotalByTorneo(int torId)
        {
            ...
            var equipos = dbApp.tblEquipos.Where(e => e.torId == torId && e.equEstatus == true)
                                          .Where(e => e.equPagado == true || e.equFechaVencimientoPagoUTC > DateTime.UtcNow).ToList();
            totales.totalEquipos = equipos.Count;
            var jugadores = dbApp.tblJugadorEquipos.Where(e => e.torId == torId && e.equId == 0 && e.jugEstatus == true)
                                              .Where(e => e.jugPagado == true || e.jugFechaVencimientoPagoUTC > DateTime.UtcNow).ToList();
        ...
        {
            var jugadores = (from jug in dbApp.tblJugadores
                       join jugE in dbApp.tblJugadorEquipos on jug.jugUserId equals jugE.jugUserId
        ...
        public List<schemaEquipos> getEquipoByAdmin(string userId)
        {
            var model = (from equ in dbApp.tblEquipos
                         ...
                         || equCo.equUserId == userId && equ.equEstatus == true
                         select equ).ToList();
            return model;
        ...
        public bool setAdminEquipo_ConfirmarParticipacion(ApplicationUser user, int torId, int equId)
        {
            try
            {
                var admin = dbApp.tblEquipos.Where(l => l.equUserIdCreador == user.Id && l.equId == equId && l.torId == torId);
                if (admin!=null)
                {
        ...
        public bool setEquipoConfirmCoadmin(ApplicationUserManager userManager, ApplicationUser user, string code)
        {
            try
            {
                ...
                if (coadmin != null)
                {
                    coadmin.equFechaConfirmacionUTC = DateTime.UtcNow;
                    coadmin.equCodigoConfirmacion = null;
        ...
        {
            var juga = (from jug in dbApp.tblJugadores
        ...
        /// <param name="jugList"></param>
        /// <returns></returns>
        public List<JugadorEquiposModel> getJugadoresByTorneo_Equipo(int torId, int equId)
        {
            return (from jug in dbApp.tblJugadores
                    ...
                    select new JugadorEquiposModel() {
                        ...
                        jugConfirmado=jugE.jugConfirmado,
                        jugEstatus=jugE.jugEstatus,
                        jugFechaVencimientoPagoUTC=(DateTime)jugE.jugFechaVencimientoPagoUTC   
                    }).ToList();
        }

        public schemaJugadores getJugadorByUserId(string jugUserId)
        {
            var jugador = dbApp.tblJugadores.Where(l => l.jugUserId == jugUserId).FirstOrDefault();
            return jugador;
        }
        ...
        /// <param name="equId"></param>
        /// <returns></returns>
        public JugadorEquiposModel getJugadorByIds(string jugCorreo, int torId, int equId)
        {
            return (from jug in dbApp.tblJugadores
                    join jugE in dbApp.tblJugadorEquipos on jug.jugUserId equals jugE.jugUserId
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public int setJugadoresEquipo_AgregarEditar(EquiposJugadoresViewModel model, string codigoConfirmacion, string userId)
        {
            try
            {
                DateTime fechaHoyUTC = DateTime.UtcNow;
                int dias = (model.tblTorneo.torDiasParaPago == null) ? 1 : (int)model.tblTorneo.torDiasParaPago;
                DateTime equiFechaVencimientoPagoUTC = DateTime.UtcNow.AddDays(dias);
              ...
              /*  var jugReintegrar = (from jugBD in jugadoresBD
                                     where (jugBD.jugPagado == false && jugBD.jugFechaVencimientoPagoUTC < DateTime.UtcNow)
                ...
                var addJug = (from jugNu in model.listJugadores
                           where jugNu.jugEstatus == true
                              && !(from jugBD in jugadoresBD
                                       where jugBD.jugEstatus == true
                                   select jugBD.jugCorreo).Contains(jugNu.jugCorreo)
                           select jugNu).ToList();
                ...
                if ( addJug != null && addJug.Count > 0 )
                {
                    ...
                    {
                        var jugador = new schemaJugadores();
                        if (item.jugUserId != null)
                            jugador = getJugadorByUserId(item.jugUserId);
                        else
                            jugador = null;
                        var id= 0;
                        if(jugador==null)
                        {
                            ...
                            jug.jugFechaCreacionUTC = DateTime.UtcNow;
                        ...
                        var jugE = dbApp.tblJugadorEquipos.Where(l => l.jugId == id && l.equId == equId).ToList();
                ...
                }else
                {
                    if (deleteJug.Count >0)
                    {
                        foreach (var item in deleteJug)
                        {
                            try
                            {
                                var jugDelete = dbApp.tblJugadorEquipos.Where(l => l.equId == equId).ToList();
                                if (jugDelete != null && jugDelete.Count > 0)
                                {
                                    var jug_delete = jugDelete.Where(l => l.jugUserId == item.jugUserId).ToList();

                                    if (jug_delete != null)
                                    {
                                        /*
                                        jug_delete.ForEach(f => f.jugEstatus = item.jugEstatus );
                                        jug_delete.ForEach(f => f.jugCodigoConfirmacion = null);
                                        jug_delete.ForEach(f => f.jugConfirmado = false);
                                        */
                                        dbApp.tblJugadorEquipos.RemoveRange(jug_delete);
                                        
                                        dbApp.SaveChanges();
                                    }
                                    else
                                    {
                                        var jugCorreo = dbApp.tblJugadores.Where(l => l.jugCorreo.Equals(item.jugCorreo)).FirstOrDefault();
                                        if (jugCorreo != null)
                                        {
                                            var jugById = jugDelete.Where(l => l.jugId.Equals(jugCorreo.jugId)).FirstOrDefault();
                                            if (jugById != null)
                                            {
                                                /*
                                                jugById.jugEstatus = item.jugEstatus;
                                                jugById.jugCodigoConfirmacion = null;
                                                jugById.jugConfirmado = false;
                                                */
                                                dbApp.tblJugadorEquipos.Remove(jugById);
                                                dbApp.SaveChanges();
                                            }
                                        }
                                    }
        ...
        public bool setJugadoresEquipo_ConfirmarParticipacion(string email, int torId, int equId,string userId,string code ,bool pay = false)
        {
            try
            {
                ...
                if (jugador != null)
                {
                    ...
                    if(jugador.jugNombre == "" || jugador.jugNombre == null)
                    {
                        jugador.jugNombre = dbApp.tblUsersProfiles.Where(l => l.userIdOwner == userId).First().uprNombres;
                    }
                    var player = new schemaJugadorEquipos();
                    var jugUserIdUpdate = new schemaJugadores();
                    jugUserIdUpdate = dbApp.tblJugadores.Where(l => l.jugCorreo == email).FirstOrDefault();
                    if (jugUserIdUpdate.jugUserId == null)
                    {                       
                        jugUserIdUpdate.jugUserId = userId;
                        dbApp.SaveChanges();
                    }
                ...
                }else
                {
                    ...
                    if (jugC==null)
                    {
                        ...
                        jug.jugCorreo = email;
                        jug.jugUserId = userId;
                        jug.jugNombre = dbApp.tblUsersProfiles.Where(l => l.userIdOwner == userId).First().uprNombres;
                        jug.jugEstatus = true;
                        jug.jugFechaCreacionUTC = DateTime.UtcNow;
                        dbApp.tblJugadores.Add(jug);
                        jugId = jug.jugId;
        ...
        public bool setJugadoresEquipo_RechazarParticipacion(string email, int torId, int equId)
        {
            ...
            {
                var jugador = getJugadorByIds(email, torId, equId);
                var jugadorE = dbApp.tblJugadorEquipos.Where(l => l.equId == equId && l.torId == torId && l.jugUserId == jugador.jugUserId).FirstOrDefault();
                if (jugadorE != null)
                {
        ...
        /// <returns></returns>
        public JugadorEquiposModel setJugadoresEquipo_VolverInvitar(string jugCorreo, int torId, int equId, string codigo)
        {
            try
            {
        ...
        public List<PartidosViewModel> getPartidosPlayer(ApplicationUser user)
        {
           ...
           // {
                ...
                partidos = (from equ in equipo
                            ...
                            join can in dbApp.tblLigaCanchasTorneos on par.lcatId equals can.lcatId
        ...
        public List<PartidosViewModel> getPartidos(ApplicationUser user)
        {
            ...
            if (user != null)
            {
                if (listRol.ToUpper() == constClass.rolOwners)
                {
                    ...
                    var liga = getPartidosByLigas(user).Where(l=> l.parCheck == false).ToList();
                    ...
                    if (cuentas.Count > 0)
                    {
                        ...
                        {                            
                            var equipos = getEquipoByPlayer(subCounts.userId);
                            if (equipos.Count > 0)
                            {
                ...
                var partidos = getListPartidos().Where(l=> l.parCheck == false).ToList();
            ...
            return getListPartidos().Where(l=> l.parCheck == true).ToList();
        }
        ...
        public List<PartidosViewModel> getListPartidos()
        {
            var partidos = (from par in dbApp.tblPartidos
                            ...
                            join can in dbApp.tblLigaCanchasTorneos on par.lcatId equals can.lcatId
                            join lcat in dbApp.tblLigaCategoriasTorneos on par.ligId equals lcat.ligId
        ...
        public List<PartidosViewModel> getPartidosEquipos(ApplicationUser user)
        {
            ...
            {
                var partidos = getPartidos(user);
                List<PartidosViewModel> equipos = new List<PartidosViewModel>();

                foreach (var par in partidos)
                {
                    PartidosViewModel item = new PartidosViewModel();
                    var equ = dbApp.tblEquipos.Where(l => l.equUserIdCreador.ToUpper() == user.Id.ToUpper()).ToList();
                    foreach (var equip in equ)
                    {
                        if (par.equIdUno == equip.equId || par.equIdDos == equip.equId)
                        {
                            item = par;
                            equipos.Add(item);
                        }
        ...
        public List<TablaGeneralTorneo> getEstadisticasTorneo(int torId, int? equId = null, int? parId=null)
        {
            ...
            if (equId != null)
            {
                result = (from par in dbApp.tblPartidos
                          ...
                          where equ.torId == torId && equ.equId == equId
            ...
            foreach (var item in result)
            {
                ...
                var partido = dbApp.tblPartidos.Where(l => l.equIdUno == item.equId || l.equIdDos == item.equId && l.parEstatus == true && l.torId == torId && l.parFecha_Fin <= DateTime.UtcNow);
                if (partido.Count()>0)
                    item.parJugados = partido.Count();

                var parGan = 0; var parEmp = 0; var parPer = 0; var golF = 0; var golC = 0;

                var partGanEmpPer = dbApp.tblPartidos.Where(l => l.equIdUno == item.equId);
                ...
                var partGanEmpPer2 = dbApp.tblPartidos.Where(l => l.equIdDos == item.equId);
        ...
        public bool setArbitroLiga(int arbId,int ligId, string codigo,bool edit=false)
        {
            try
            {
                if (edit==true)
                {
                    var arbLiga = dbApp.tblArbitrosLigas.Where(l=> l.arbId == arbId).FirstOrDefault();
                    arbLiga.ligId=ligId;
                    if (codigo!="")
                    {
        ...
        public List<PagosGridViewModel> getGridPagos(ApplicationUser user ,bool admin=false)
         {
            ...
            else if (rolLiga == rolActual)
            {
                ...
                foreach (var lig in ligas)
                {
                    ...
                    {
                        view.fechaPago = (lig.statusPago == "paid") ? DateTime.UtcNow.ToString() : lig.ligFechaRegistroUTC.ToString();
                    }
        ...
        public List<PagosGridViewModel> getGridPagosTorneos(string userId)
        {
            var pendientes = (from tor in dbApp.tblTorneos
                           ...
                           from tca in tor_tca.DefaultIfEmpty()
                           where (tor.torUserIdCreador == userId
                               || tca.tcaUserId == userId) && tor.torPagado == false
                           select tor)
                                .Distinct()
                                .ToList();
            ...
            var pagos = (from pag in dbApp.tblPagos
                         ...
                         where pag.concepto == "Torneo" &&
                               ...
                               || tca.tcaUserId == userId) && tor.torPagado == true
                         select new PagosGridViewModel()
                         {
        ...
        public List<PagosGridViewModel> getGridPagosEquipo()
        {
            var pagos = (from pag in dbApp.tblPagos
                         ...
                         select new PagosGridViewModel()
                         {
                             ...
                             userName = equ.tblUsuarioCreador.UserName,
            ...
            foreach (var item in pendientes)
            {
                ...
                aux.userName = item.tblUsuarioCreador.UserName;
        ...
        public bool setEquipoPago(int equId, bool status)
        {
            try
            {
